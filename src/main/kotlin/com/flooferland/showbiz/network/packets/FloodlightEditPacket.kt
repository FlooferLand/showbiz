...
class FloodlightEditPacket(editScreen: EditScreenMenu.EditScreenBuf, var turn: Float, var angle: Float, var shadows: Boolean, var color: Int, var brightness: Float) : EditScreenMenu.EditScreenPacketPayload(editScreen) {
    ...
    companion object {
        ...
        val codec = StreamCodec.of<FriendlyByteBuf, FloodlightEditPacket>(
            ...
            { buf ->
                ...
                val turn = buf.readFloat()
                val angle = buf.readFloat()
                val shadows = buf.readBoolean()
                val color = buf.readInt()
                val brightness = buf.readFloat()
                FloodlightEditPacket(editScreen, turn = turn, angle = angle, shadows = shadows, color = color, brightness = brightness)
            }
        )!!
    }
}