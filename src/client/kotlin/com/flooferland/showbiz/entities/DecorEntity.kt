    ...
    companion object {
        ...
        val decorTick = object : StagedBotBlockEntity.IDecor {
            override fun tick(owner: StagedBotBlockEntity, level: Level, pos: BlockPos, state: BlockState) {
                ...
                if (botId.matches("showbiz:mitzi_mozzarella") && entities.count { it.decorId == Id.PomPom && it.boneName!!.startsWith("Hand") } != 2) {
                    spawn(owner, level, "HandL", Id.PomPom)
                    spawn(owner, level, "HandR", Id.PomPom)
                }
            }
        }
