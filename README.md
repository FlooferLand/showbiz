# Showbiz

![Created at](https://img.shields.io/github/created-at/FlooferLand/Showbiz-Mod?style=plastic&logo=github&link=https://github.com/FlooferLand/Showbiz-Mod)
![Code size in bytes](https://img.shields.io/github/languages/code-size/FlooferLand/showbiz?style=plastic&color=brightgreen&link=https://github.com/FlooferLand/showbiz)
![Issue tracker](https://img.shields.io/github/issues/FlooferLand/showbiz?style=plastic&link=https://github.com/FlooferLand/showbiz/issues)

> [!WARNING]  
> Showbiz is in VERY early development, expect things to break and be unfinished!

A fan-made Minecraft mod focused around re-creating the wonderful experience that is watching [The Rock-afire Explosion](https://www.youtube.com/watch?v=MyEbr6HvWy0) perform,
or any other band via it's addon system!

I made this since Minecraft lets you combine tons of different mods and situations, making this simulator able to be enjoyed in VR via mods like ViveCraft, or with multiplayer with friends.

_Not affiliated nor associated with Aaron Fechter, Rock-afire Explosion, or Creative Engineering. This is purely a fan project._

Note that this project is split into several different components:
- [Showbiz Mod](https://github.com/FlooferLand/showbiz) - The Minecraft mod
- [Showbiz (VSCode)](https://github.com/FlooferLand/showbiz-vscode) - Visual Studio Code extension aimed at addon developers
- [BizLib](https://github.com/FlooferLand/bizlib) - Handles format reading, parsing, and other low level things.
  - [BizLibNative](https://github.com/FlooferLand/bizlib/tree/main/BizlibNative) - C#-based library I wrote to maximize engine compatibility, as it reads shows roughly the same way RR Engine does _(except using NRBF instead of BinaryFormatter since it's far more secure)_


## Building

1. Download this project 
   - If you want to just quickly download it and delete it after, you can click the green `< > Code` button in the top right of the page.
   - Otherwise, I recommend cloning it using [git](https://git-scm.com) via `git clone https://github.com/FlooferLand/showbiz` <br/>
     You can then easily update the source code by running `git pull` in the project directory
   - If you want to help develop Showbiz and contribute your changes, I recommend [forking it](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/working-with-forks/fork-a-repo#forking-a-repository)
     then following the previous instructions, but replacing `FlooferLand` in the URL with your own GitHub username.
     This makes it so you have your own clone of Showbiz you can modify, then on your fork's page you can create a pull request to submit your changes over to this repo.
     Do note that you have to manually update your fork's repository to receive updates from this repository.

2. Open the project in [IntelliJ IDEA](https://www.jetbrains.com/idea/download), then open this `README.md` in it and continue reading from there on

3. Build the project by running `gradle build` _<-- IntelliJ should be showing a play button next to this text in the preview window, click it_

4. Decompile Minecraft via `gradle genSources`

5. Build the mod assets via `gradle runDatagen`

You've done it, you built the mod!

Now, if you want to assemble it into a jar file, run `gradle assemble`, then look inside `versions\<minecraft_version>\build\libs` when it's done. Grab the file that doesn't end in anything funny _(shouldn't have `-all` at the end of its name or anything)_

If you want to run the project inside Intellij itself, you can select the `Minecraft Client` profile in the top right and press the play button.

Just note that this is the latest in-development version of Showbiz, which is not stable whatsoever.
By using it, your worlds ***WILL*** be at risk of breaking.
Regularly back up worlds using Minecraft's `Edit > Make Backup` button.


## Contributing

This is an open-source project, so I'm glad if you can help!

If you'd like to add anything to the project, please open an issue or send a message on our Discord server to talk about it with us _(linked on the [Modrinth](https://modrinth.com/mod/showbiz) page)_.

Make sure you're at least somewhat familliar with the project's internals before doing so if trying to submit a technical change.


## Addons
Showbiz allows you to add in your own bots, among other things.

Please see [the wiki](https://github.com/FlooferLand/showbiz/wiki) on more information about how to do this.
